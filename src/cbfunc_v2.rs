#![deprecated(note = "one-off binary bruteforcer (hardcoded target + DEPTH); superseded by eml_v5_1")]
...
fn main() {
    ...
    for i in 0..total {
        ...
        if i % 10_000_000 == 0 && i > 0 {
            ...
            writeln!(
                ...
                i, total, pct, rate, remaining, matched, validity_cache.len()
            ).unwrap();
            progress.flush().unwrap();
        }
    }
    ...
    writeln!(
        ...
        elapsed, matched, validity_cache.len()
    ).unwrap();
    progress.flush().unwrap();
